...
class DetectionCheckpointer(Checkpointer):
    ...
    def _torch_load(self, f):
        try:
            ...
            # rejects older checkpoints that embed config objects such as
            # OmegaConf containers. Retry trusted local checkpoints with the
            # legacy behavior to preserve detectron2/detrex compatibility.
            self.logger.warning(
                "Retrying checkpoint load with weights_only=False for %s due to "
                "PyTorch 2.6 weights-only deserialization restrictions.",
                f,
            )
