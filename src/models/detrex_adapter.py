...
def load_daod_model_config(cfg: Any, *, require_checkpoint: bool = True) -> DetrexAdapterConfig:
    ...
    return DetrexAdapterConfig(
        ...
        init_checkpoint=str(_get_checkpoint_path(detector_cfg, model_name)) if require_checkpoint else None,
...
def build_daod_model(cfg: Any, *, load_weights: bool = True, device: str | torch.device | None = None) -> DetrexAdapter:
    sys.path.insert(0, str(_DETREX_ROOT))

    adapter_cfg = load_daod_model_config(cfg, require_checkpoint=load_weights)
    ...
    lazy_cfg.train.device = resolved_device
    lazy_cfg.model.device = resolved_device
...
def _prepare_input(adapter: DetrexAdapter, sample: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "image": torch.as_tensor(image, dtype=torch.float32, device=model_device),
        "height": height,
        "width": width,
