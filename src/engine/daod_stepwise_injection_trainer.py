...
from src.engine.utils import save_json, seed_everything
...
def _build_stage_loaders(
    ...
    *,
    ...
    def seed_worker(worker_id: int) -> None:
        worker_seed = (int(seed) + 1009 * int(stage_idx) + int(worker_id)) % (2**32)
        random.seed(worker_seed)
        np.random.seed(worker_seed)

    labeled_generator = torch.Generator()
    labeled_generator.manual_seed(int(seed) + 2003 * int(stage_idx) + 17)
    unlabeled_generator = torch.Generator()
    unlabeled_generator.manual_seed(int(seed) + 2003 * int(stage_idx) + 53)
...
class DAODStepwiseInjectionTrainer:
    ...
    def fit_with_injections(
        ...
        *,
        ...
        stability_cfg = getattr(method_train_cfg, "stability", object())
        ...
        grad_accum_steps = max(1, int(getattr(method_train_cfg, "grad_accum_steps", 1)))
        ...
        if use_pseudo_labels:
            micro_steps_per_epoch = max(full_labeled_len, full_unlabeled_len, 1)
        else:
            micro_steps_per_epoch = max(full_labeled_len, 1)
        total_micro_steps = max_epochs * micro_steps_per_epoch
        total_steps = max(1, int(math.ceil(float(total_micro_steps) / float(grad_accum_steps))))
        ...
        pseudo_score_calibration_cfg = getattr(method_train_cfg, "pseudo_score_calibration", object())
        pseudo_score_calibration_enabled = bool(getattr(pseudo_score_calibration_cfg, "enabled", False))
        pseudo_score_calibration_replace_score = bool(getattr(pseudo_score_calibration_cfg, "replace_score", True))
        pseudo_reliability_cfg = getattr(method_train_cfg, "pseudo_reliability_weighting", object())
        ...
        clip_max_norm = max(0.0, float(getattr(method_train_cfg, "clip_max_norm", 0.0)))
        pseudo_cache_cfg = getattr(method_train_cfg, "pseudo_cache", object())
        if bool(getattr(pseudo_cache_cfg, "enabled", False)):
            raise ValueError(
                "method.train.pseudo_cache has been removed because stage-wise hard pseudo caching hurt performance. "
                "Disable pseudo_cache in the config."
            )
        ...
        if pseudo_score_calibration_enabled:
            score_calibrator, score_calibration_stats = fit_pseudo_score_calibrator(
                target_train_dicts,
                labeled_ids,
                num_classes=num_classes,
                calibration_cfg=pseudo_score_calibration_cfg,
                teacher_adapter=teacher_adapter,
                stage_idx=0,
                seed=cfg_seed,
            )
            save_json(run_dir / "pseudo_score_calibration_stage_0.json", score_calibration_stats)
        ...
        labeled_loader, unlabeled_loader, labeled_iter, unlabeled_iter, _, unlabeled_items = _build_stage_loaders(
            target_train_dicts,
            labeled_ids,
        ...
        _append_jsonl(
            injection_log_path,
            {
                ...
                "pseudo_score_calibration": score_calibration_stats if pseudo_score_calibration_enabled else None,
            },
        )
        ...
        aema_grad_accum: dict[str, torch.Tensor] = {}
        ...
        def maybe_inject() -> None:
            ...
            nonlocal score_calibrator, score_calibration_stats
            ...
            if pseudo_score_calibration_enabled:
                ...
                save_json(run_dir / f"pseudo_score_calibration_stage_{current_stage + 1}.json", score_calibration_stats)
            stage_idx = current_stage + 1
            labeled_loader, unlabeled_loader, labeled_iter, unlabeled_iter, _, unlabeled_items = _build_stage_loaders(
                target_train_dicts,
                labeled_ids,
        ...
        try:
            student_model.train()
            strong_view_rng = random.Random(cfg_seed + 4241)
            epoch_stats = {"loss_total": 0.0, "loss_sup": 0.0, "loss_hard": 0.0, "loss_soft": 0.0, "steps": 0.0}
            for optimizer_step_idx in range(total_steps):
                maybe_inject()
                accum_steps_this_update = min(grad_accum_steps, total_micro_steps - optimizer_step_idx * grad_accum_steps)
                if accum_steps_this_update <= 0:
                    break
                ...
                log_loss_hard = 0.0
                log_loss_soft = 0.0
                log_labeled_images = 0
                log_unlabeled_images = 0
                log_hard_pseudo_count = 0
                log_soft_target_count = 0
                log_hard_reliability_weight_sum = 0.0
                log_hard_reliability_weight_count = 0
                log_soft_reliability_weight_sum = 0.0
                log_soft_reliability_weight_count = 0
                micro_steps = 0

                for _accum_idx in range(accum_steps_this_update):
                    ...
                    if labeled_batch:
                        labeled_inputs = _make_supervised_inputs(
                            ...
                            max_size=max_size,
                            device=self.device,
                            strong_view_rng=strong_view_rng,
                        )
                    ...
                    if unlabeled_batch and use_pseudo_labels and (enable_hard_pseudo or enable_soft_pseudo):
                        ...
                        if pseudo_score_calibration_enabled:
                            teacher_items = apply_pseudo_score_calibrator_to_items(
                                teacher_items,
                                score_calibrator,
                                replace_score=pseudo_score_calibration_replace_score,
                            )
                        student_items = (
                            _student_outputs_for_unlabeled(
                                student_adapter,
                            ...
                            else [None for _ in teacher_items]
                        )
                        _append_jsonl(
                            memory_log_path,
                            _mem_log_payload(self.device, tag="after_unlabeled_forward", epoch=1, step=global_step + 1),
                        )
                        for teacher_item, student_item in zip(teacher_items, student_items):
                            hard_rows = []
                            if enable_hard_pseudo:
                                ...
                                if hard_rows:
                                    ...
                                    if pseudo_annotations:
                                        hard_reliability_weight = 1.0
                                        if pseudo_reliability_enabled:
                                            hard_reliability_weight, _ = pseudo_reliability_weight_for_rows(
                                                hard_rows,
                                                pseudo_reliability_cfg,
                                                thresholds=hard_class_score_mins,
                                            )
                                        pseudo_sample = dict(teacher_item["sample"])
                                        pseudo_sample["annotations"] = pseudo_annotations
                                        pseudo_sample["_pseudo_reliability_weight"] = hard_reliability_weight
                                        pseudo_sample["_pseudo_reliability_num_rows"] = len(hard_rows)
                                        hard_batch.append(pseudo_sample)
                            elif enable_soft_pseudo:
                                hard_rows = _build_hard_teacher_rows(
                                    teacher_item,
                                    hard_score_min=hard_score_min,
                                    hard_nms_iou=hard_nms_iou,
                            ...
                            if enable_soft_pseudo and student_item is not None:
                                ...
                                if soft_targets:
                                    soft_reliability_weight = 1.0
                                    if pseudo_reliability_enabled:
                                        soft_reliability_weight, _ = pseudo_reliability_weight_for_rows(
                                            [target["teacher_row"] for target in soft_targets],
                                            pseudo_reliability_cfg,
                                            thresholds=None,
                                        )
                                    soft_batch.append(
                                        {
                                            ...
                                            "_pseudo_reliability_weight": soft_reliability_weight,
                                            "_pseudo_reliability_num_rows": len(soft_targets),
                                        }
                                    )

                    if enable_hard_pseudo and hard_batch:
                        hard_reliability_weight = (
                            pseudo_reliability_weight_for_samples(hard_batch)
                            if pseudo_reliability_enabled
                            else 1.0
                        )
                        ...
                        loss_hard = hard_reliability_weight * hard_loss_weight * sum(hard_loss_dict.values())
                        loss = loss + loss_hard
                        log_hard_reliability_weight_sum += float(hard_reliability_weight)
                        log_hard_reliability_weight_count += 1
                        _append_jsonl(
                            memory_log_path,
                            _mem_log_payload(self.device, tag="after_hard_loss", epoch=1, step=global_step + 1),
                        )
                    if enable_soft_pseudo and soft_batch:
                        soft_reliability_weight = (
                            pseudo_reliability_weight_for_samples(soft_batch)
                            if pseudo_reliability_enabled
                            else 1.0
                        )
                        loss_soft = soft_reliability_weight * _student_soft_loss(
                            soft_batch,
                            soft_loss_weight=soft_loss_weight,
                            match_iou_min=soft_match_iou_min,
                        )
                        loss = loss + loss_soft
                        log_soft_reliability_weight_sum += float(soft_reliability_weight)
                        log_soft_reliability_weight_count += 1
                        _append_jsonl(
                            memory_log_path,
                            _mem_log_payload(self.device, tag="after_soft_loss", epoch=1, step=global_step + 1),
                        )

                    if loss.requires_grad and float(loss.detach().cpu()) > 0.0:
                        (loss / float(accum_steps_this_update)).backward()
                        did_backward = True

                    log_loss_total += float(loss.detach().cpu())
                    log_loss_sup += float(loss_sup.detach().cpu())
                    log_loss_hard += float(loss_hard.detach().cpu())
                    log_loss_soft += float(loss_soft.detach().cpu())
                    log_labeled_images += int(len(labeled_batch))
                    log_unlabeled_images += int(len(unlabeled_batch))
                    log_hard_pseudo_count += int(hard_pseudo_count)
                    log_soft_target_count += int(soft_target_count)

                if did_backward:
                    if clip_max_norm > 0.0:
                        torch.nn.utils.clip_grad_norm_(student_model.parameters(), clip_max_norm)
                    if ema_mode == "aema":
                        _accumulate_grad_importance(aema_grad_accum, student_model)
                    optimizer.step()
                ...
                denom_micro = max(micro_steps, 1)
                avg_loss_total = log_loss_total / denom_micro
                avg_loss_sup = log_loss_sup / denom_micro
                avg_loss_hard = log_loss_hard / denom_micro
                avg_loss_soft = log_loss_soft / denom_micro
                avg_hard_reliability_weight = (
                    log_hard_reliability_weight_sum / max(log_hard_reliability_weight_count, 1)
                )
                avg_soft_reliability_weight = (
                    log_soft_reliability_weight_sum / max(log_soft_reliability_weight_count, 1)
                )
                epoch_stats["loss_total"] += avg_loss_total
                epoch_stats["loss_sup"] += avg_loss_sup
                epoch_stats["loss_hard"] += avg_loss_hard
                epoch_stats["loss_soft"] += avg_loss_soft
                ...
                if log_period > 0 and global_step % log_period == 0:
                    _append_jsonl(
                        train_log_path,
                        {
                            ...
                            "loss_total": float(avg_loss_total),
                            "loss_sup": float(avg_loss_sup),
                            "loss_hard": float(avg_loss_hard),
                            "loss_soft": float(avg_loss_soft),
                            "num_labeled_images": int(log_labeled_images),
                            "num_unlabeled_images": int(log_unlabeled_images),
                            "num_hard_pseudo_boxes": int(log_hard_pseudo_count),
                            "num_soft_targets": int(log_soft_target_count),
                            "current_stage": int(current_stage),
                            "grad_accum_steps": int(grad_accum_steps),
                            "accum_steps_this_update": int(accum_steps_this_update),
                            ...
                            "pseudo_score_calibration_enabled": bool(pseudo_score_calibration_enabled),
                            "pseudo_score_calibration_method": score_calibration_stats.get("method") if pseudo_score_calibration_enabled else None,
                            "pseudo_score_calibration_fallback": bool(score_calibration_stats.get("fallback_to_identity", False))
                            if pseudo_score_calibration_enabled
                            else None,
                            "pseudo_score_calibration_replace_score": bool(pseudo_score_calibration_replace_score)
                            if pseudo_score_calibration_enabled
                            else None,
                            "pseudo_reliability_weighting_enabled": bool(pseudo_reliability_enabled),
                            "hard_pseudo_reliability_weight": float(avg_hard_reliability_weight)
                            if pseudo_reliability_enabled
                            else None,
                            "soft_pseudo_reliability_weight": float(avg_soft_reliability_weight)
                            if pseudo_reliability_enabled
                            else None,
                        },
                    )
            ...
            return {
                ...
                "total_micro_steps": int(total_micro_steps),
