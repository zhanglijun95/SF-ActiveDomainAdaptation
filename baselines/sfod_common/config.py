"""Config and checkpoint path helpers for isolated SFOD baselines."""
...
def resolve_sfod_run_dir(cfg: Any, *, baseline_key: str) -> Path:
    ...
    return root / "baselines" / baseline_key / exp_tag / f"{source}__to__{target}" / model_name


def resolve_daod_source_ckpt_path(cfg: Any, which: str = "final") -> Path:
    explicit = Path(str(which)).expanduser()
    if explicit.suffix == ".pth" or explicit.exists():
        return explicit
    ...
    raise ValueError(f"Unsupported DAOD source checkpoint selector: {which}. Use one of: best, final, or a .pth path")
