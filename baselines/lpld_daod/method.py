"""Entrypoint orchestration for the isolated LPLD DAOD baseline."""
...
from .config import resolve_lpld_daod_run_dir, resolve_lpld_daod_source_ckpt_path


class LPLDDAODMethod:
    def __init__(self, cfg: Any, device: torch.device) -> None:
        self.cfg = cfg
        self.device = device
        self.run_dir = resolve_lpld_daod_run_dir(cfg)
    ...
    def run(self, *, config_path: str | Path | None = None) -> dict[str, Any]:
        ...
        source_ckpt = resolve_lpld_daod_source_ckpt_path(
            self.cfg,
            which=str(getattr(getattr(self.cfg, "detector", object()), "source_ckpt", "final")),
        )
        if not source_ckpt.exists():
            raise FileNotFoundError(
                f"Missing DAOD source checkpoint for LPLD baseline: {source_ckpt}\n"
                "Run source DINO training first, or set detector.source_ckpt to an existing checkpoint."
            )

        trainer = SFODBaselineTrainer(cfg=self.cfg, device=self.device, algorithm="lpld", log_prefix="LPLD-DAOD")
