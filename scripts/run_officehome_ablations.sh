...
CFG_DIR="$ROOT_DIR/configs/ablations/office_home"
LOG_DIR="$ROOT_DIR/runs/logs/ablations/office_home"
mkdir -p "$LOG_DIR"
...
COMMANDS_FILE="$ROOT_DIR/scripts/run_officehome_ablations.commands.txt"
{
  echo "# Smoke command (first ablation)"
  echo "$SMOKE_CMD"
  echo
  echo "# Full Office-Home ablation jobs"
  for i in "${!JOBS[@]}"; do
...
Modes:
  ...
  --run-all Run all Office-Home ablations in parallel, logs in $LOG_DIR
